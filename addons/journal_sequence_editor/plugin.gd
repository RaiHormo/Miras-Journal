...
var view_instance: Control

func _enter_tree() -> void:
	view_instance = SEQUENCE_VIEW.instantiate()
	# Add the main panel to the editor's workspace
	get_editor_interface().get_editor_main_screen().add_child(view_instance)
	_make_visible(false)

func _exit_tree() -> void:
	if view_instance:
		get_editor_interface().get_editor_main_screen().remove_child(view_instance)
